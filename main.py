...
# experiment config
parser.add_argument('--env-name', default="metadrive",
                    help='Metadrive environment (default: HalfCheetah-v2)')
...
parser.add_argument('--experiment-name', type=str, default="metadrive_lwf")
...
parser.add_argument('--final-test-times', type=int, default=5000)
...
parser.add_argument('--load-model-path', type=str, default="",#checkpoints/metadrive/metadrive_lwf/agent_20",
...
parser.add_argument('--num-steps', type=int, default=300000, metavar='N',
...
''' Data Config '''
# data config
parser.add_argument('--load-data-path', type=str, default="data/metadrive/metadrive",
                    help='load offline data path')
parser.add_argument('--pretrain-data-size', type=int, default=200000,
...
parser.add_argument('--not-pretrain-recovery', default=False, action='store_true',
                    help='pretrain model')
parser.add_argument('--pretrain-steps-recovery', type=int, default=300000,
                    help='pretrain steps (default: 500000)')
parser.add_argument('--save-pretrain-model-path', type=str, default='./pretrain_models/metadrive',
                    help='save pretrain model path')
parser.add_argument('--load-pretrain-model-path', type=str, default="/pretrain_models/metadrive/metadrive",
                    help='load pretrain model path')
parser.add_argument('--pretrain-model-name', type=str, default="pretrain_model1")


# finetune config
parser.add_argument('--not-finetune', default=False, action='store_true')
parser.add_argument('--eps-safe', type=float, default=0.7,
...
# collect data
parser.add_argument('--collect-data-only', default=False, action='store_true')
...
if __name__ == '__main__':
    ...
    if args.collect_data_only == True:
        args.not_train = True
        args.not_pretrain = True
        args.use_recovery = False
        args.batch_size_test = 20
    if args.train_only == True:
        args.not_train = False
        args.not_pretrain = True
        args.use_recovery = False
        args.batch_size_run = 10
